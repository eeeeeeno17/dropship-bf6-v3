...
void App::render(bool* p_open) {
	...
	ImGui::Begin("dropship [modified by dcbf]", p_open, window_flags);

	{
		/* draw */

		//if (g_updater) (*g_updater).render(); // Removed update button
...
void App::renderError(std::string error) {
	...
	/* twitter */
	{
		...
		ImGui::PushStyleColor(ImGuiCol_ButtonHovered, color_button_hover);
		{
			if (ImGui::Button("[forked]")) {
				// Forked version - no social link
			}
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0, 0, 0, 1));
			ImGui::SetItemTooltip("Forked from original project");
			ImGui::PopStyleColor(1);
