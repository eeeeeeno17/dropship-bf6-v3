    ...
    pub fn known_servers(&self) -> &[api::KnownServer] {
        if let Some(cache) = &self.cache {
            if let Some(data) = &cache.cached_api_data {
                return data.servers.bf6.as_slice();
            }
        }
        &[]
    }
    ...
    fn ui(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
                ...
                egui::Sides::new().show(
                    ...
                    |ui| {
                        ...
                        ui.with_layout(egui::Layout::top_down(egui::Align::LEFT), |ui| {
                            ...
                            ui.label("written by stormy, modified by DCBF");
                        });
                        // });
                        // });
                    },
            ...
            .show(ui, |ui| {
                ...
                egui::Panel::bottom("actions")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.scope(|ui| {
                            let button = egui::Button::new("Dropship 비활성화");
                            let button =
                                ui.add_sized(egui::vec2(ui.available_width() - 8. - 4., 16.0), button)
                                    .on_hover_text("Dropship의 모든 차단을 취소합니다.");
        ...
        if !self.config.mini {
            ...
            .show(ui, |ui| {
                ...
                ui.heading(" 플레이 하지 않을 서버를 클릭하여 차단하세요 -> ->");
                ...
                ui.label("이 설정은 위에서 선택한 애플리케이션에 적용됩니다. 이 창을 계속 열어둘 필요는 없습니다. 단, 'Dropship이 켜져있는 동안' 설정을 사용하는 경우에는 창을 계속 열어두어야 합니다. 서버에 마우스 커서를 올리면 해당 서버의 대략적인 지연 시간을 확인할 수 있습니다.");
...
impl TemplateApp {
    ...
    fn applications(&mut self, ui: &mut egui::Ui) {
            ...
            ui.horizontal(|ui| {
                ui.scope(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        // }
                        {
                            let button = egui::Button::new("{{ 추가하기 }}")
        ...
        if let Some(path) = &self.modal_manage_path {
            ...
            let modal = egui::Modal::new(egui::Id::new("export_ips")).show(ui.ctx(), |ui| {
                ...
                {
                    let button = egui::Button::new("로컬에서 열기");
                ...
                if let Some(known_paths) = self.config.known_paths.as_mut() {
                    {
                        let button = egui::Button::new("목록에서 제거");
                        let button = ui.add_sized(egui::vec2(ui.available_width(), 16.0), button);
            ...
            ui.vertical(|ui| {
                ui.scope(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        {
                            let tabs = [
                                (Some(assets::ICON_MAPLE_LEAF), "환영합니다"),
                                (Some(assets::ICON_TERMINAL), "로그"),
                                (Some(assets::ICON_HEART), "도움"),
                                (Some(assets::ICON_GEARS), "설정"),
                            ];
                            let len = tabs.len();
    ...
    fn help_wizard(&mut self, ui: &mut egui::Ui) {
        ui.label("작동하지 않는다면 배틀필드 시리즈 갤러리로 오십시오.");
        ui.horizontal(|ui| {
            ui.label("  •");
            // ui.hyperlink_to("discord", dropship::EEEEEENO);
            ui.hyperlink(dropship::DCBF_LINK);
        });
        ui.separator();
        ...
        ui.label("배틀필드 6 서버 접속기 소스 코드");
        ui.horizontal(|ui| {
            ui.label("  •");
            // ui.hyperlink_to("discord", dropship::DISCORD_INVITE_LINK);
            ui.hyperlink(dropship::GITHUB2_URI);
        });
        ...
        ui.vertical(|ui| {
            ...
            egui::Panel::right("xx")
                ...
                .show(ui, |ui| {
                    // export ips
                    {
                        if ui.button("차단된 ip 출력").clicked() {
                    ...
                    ui.separator();
                    if ui
                        .link("클릭하여 Windows 방화벽 설정을 기본값으로 되돌립니다.")
                        .clicked()
                    {
                    ...
                    ui.separator();
                    if ui.link("클릭하여 Windows DNS를 Flush 합니다.").clicked() {
...

            {
                ...
                if ui
                    .checkbox(&mut is_checked, "앱 시작시에 로그 열기")
                    .changed()
                {
    ...
    fn welcome(&mut self, ui: &mut egui::Ui, page: u8) {
        ...
        let modal = egui::Modal::new(egui::Id::new("welcome")).show(ui.ctx(), |ui| {
            ...
            match page {
                0 => {
                    ui.heading("배틀필드 6 서버 접속기");
                    ui.label(
                        "이 앱을 사용하면 어떤 서버에서 플레이할지 직접 선택할 수 있습니다.",
                    );

                    ui.separator();
                    ui.label("이 앱은...");
                    ui.indent("xd", |ui| {
                        ui.label("• 게임 파일을 수정하지 않습니다.");
                        ui.label("• EA 약관을 위반하지 않습니다.");
                    });

                }
                1 => {
                    ui.heading("어떻게 작동하나요?");

                    ui.label("플레이하고 싶지 않은 서버를 *차단*하는 방식으로 원하는 서버를 선택할 수 있습니다.");
                    ui.indent("xd4", |ui| {
                        ui.label("• 앱을 켜둘 필요가 없습니다.");
                        ui.label("• 차단은 실행 취소하기 전까지 유지됩니다.");
                    });
                }
                _ => {
                    // ui.heading("done");

                    ui.label("작동하지 않을 경우 배틀필드 시리즈 갤러리로 오십시오.");
                    ui.horizontal(|ui| {
                        ui.label("  •");
                        // ui.hyperlink_to("discord", dropship::EEEEEENO_LINK);
                        ui.hyperlink(dropship::DCBF_LINK);
                    });
    ...
    fn dynamic_wfp_dropdown(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ComboBox::from_label("서버 차단")
